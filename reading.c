...
/* Reads all words from filename and return them in a 2D array. */
char **read_words(char *filename) {
    ...
    while (fgets(buffer, MAX_WORD_LENGTH, fp)) {
        ...
        /* strcpy is safe because just allocated exactly enough space. */
...
/* Deallocates all memory acquired by read_words. */
